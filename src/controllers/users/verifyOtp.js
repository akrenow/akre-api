...
const { USER_NOT_FOUND, LOGIN_SUCCESS, INVALID_CREDENTIALS, OTP_EXPIRED } = require("../../utils/strings");
...
const verifyOtpAndLogin = async (req, res) => {
  ...
  if (!email || !otp) {
    return res.status(BAD_REQUEST_CODE).json({ success: false, message: "Email and OTP are required." });
  }
  ...
  if (user.otpExpiresAt && currentTime > user.otpExpiresAt) {
    return res.status(BAD_REQUEST_CODE).json({ success: false, message: OTP_EXPIRED });
  }

    if (!verifyOtp(otp, user.otp)) {
      return res.status(BAD_REQUEST_CODE).json({ success: false, message: "Invalid OTP." });
    }
